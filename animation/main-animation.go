	...
	bl.Root()
	{
		...
		bl.Div()
		{
			...
			bl.On("click", func(interface{}) {
				animation.StartPath(nodeId, "one", 100, 600, 10, "linear", func(shadow *bl.ShadowNode, value float32) {
					shadow.Left = int32( value)
				})

				animation.StartPath(nodeId, "two", 1, .05, 20, "linear", func(shadow *bl.ShadowNode, value float32) {
